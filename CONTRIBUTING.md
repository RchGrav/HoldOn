# Contributing to sigmund

sigmund is a small, security-sensitive C11 tool. The bar is: a change can be
...
| **The root / sudoers / private-store lane** | `scripts/linux.sh root` | runs the suite as root with `SIGMUND_REQUIRE_ROOT_TESTS=1`, exercising the privilege-delegation tests the user lane can only skip. |

`make ci` ≡ `scripts/ci.sh`; `make lint` ≡ `scripts/lint_layers.sh`. GitHub CI
calls the same scripts, so "green locally" means the same thing as "green in CI".
...
- Tests live in `tests/test_sigmund.sh` (black-box: they run the built binary and
...
- There is a real **SKIP** state. A test that needs something absent (a root
  ...
  vacuous PASS. Set `SIGMUND_REQUIRE_ROOT_TESTS=1` (CI's root lane does) to turn
...
Source is layered under `src/` with public APIs in `include/sigmund/`:
...
this. All cross-module symbols are `sigmund_`-prefixed.
...
- `do_signal_action` has a `pgid <= 1` guard that is unreachable because
  `sigmund_valid_record` already rejects `pgid <= 1` at load; it can be dropped
