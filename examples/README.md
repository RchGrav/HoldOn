...
Installs or locates `uv` and `hold`, starts a Python HTTP server through On Hold, creates a durable alias from the recorded run, stops the first run, then starts the alias from a different directory to prove the alias recipe is independent of the script's current working directory.
