# hold
[![CI](https://github.com/RchGrav/hold/actions/workflows/ci.yml/badge.svg?branch=main)](https://github.com/RchGrav/hold/actions/workflows/ci.yml)

What comes to mind when you hear the name _hold_?

Maybe a certain Viennese doctor. We won't dwell on him, though we'll concede that a man so fixated on what goes wrong between parents and children is oddly on-theme for a tool that keeps your child processes from being orphaned. Or maybe you land on Saturday mornings and Sid and Marty Krofft's _On Hold and the Sea Monsters_, the little monster who couldn't bring himself to scare anyone. We'll take that one. We do keep track of `sid`s, session IDs, and we try hard not to become a C-monster. Pun intended.

The real meaning is older than both. _Hold_ is Old English for guardianship: a protector's care over what's in its keeping. _Sig_ is the signals the kernel uses to start, stop, and watch over your processes. Together they're the whole job: a guardian for the things you launch.
...
More install modes are in [Installing On Hold](docs/install.md).
...
- name: Show helper log on failure
  if: failure()
  run: hold dump "$HELPER_RUN_ID" || true

- name: Stop helper
  if: always()
  run: |
    hold stop "$HELPER_RUN_ID" || true
    hold prune "$HELPER_RUN_ID" || true
```

Run a small local development stack:

```bash
frontend_id="$(hold npm run dev:frontend)"
backend_id="$(hold npm run dev:backend)"

hold list
hold tail "$backend_id"
hold stop "$frontend_id" "$backend_id"
```
...
On Hold is not a service supervisor. It does not restart processes after reboot, keep a resident daemon, or continuously refresh state in the background. It records enough information at launch to make later inspection and cleanup safer.
