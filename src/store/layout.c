...
static bool id_collides_in_store(const struct hold_store *store, const char *id, bool include_public);
static bool id_material_collides_in_store(const struct hold_store *store, const char *id, bool include_public);
...
static bool id_collides_in_store(const struct hold_store *store, const char *id, bool include_public) {
    ...
    if (hold_checked_snprintf(path, sizeof(path), "%s/%s.json", store->record_dir, id) == 0 && hold_path_exists(path)) {
        return true;
    }
    if (hold_checked_snprintf(path, sizeof(path), "%s/%s.log", store->log_dir, id) == 0 && hold_path_exists(path)) {
        return true;
    }
    if (hold_checked_snprintf(path, sizeof(path), "%s/.%s.reserve", store->record_dir, id) == 0 && hold_path_exists(path)) {
        return true;
    }
    ...
    if (include_public && store->public_dir[0]) {
        if (hold_checked_snprintf(path, sizeof(path), "%s/%s.json", store->public_dir, id) == 0 && hold_path_exists(path)) {
            return true;
        }
    }
    return false;
...
int hold_gen_id_for_store(const struct hold_store *primary,
    ...
    for (int tries = 0; tries < 100; tries++) {
        if (hold_rand_bytes(b, sizeof(b)) != 0) {
            return -1;
        }
        ...
        if (avoid_public_store && avoid_public_store->public_dir[0]) {
            char p[HOLD_PATH_MAX];
            if (hold_checked_snprintf(p, sizeof(p), "%s/%s.json", avoid_public_store->public_dir, out) == 0 && hold_path_exists(p)) {
                continue;
            }
        }
        ...
        if (hold_checked_snprintf(reserve, sizeof(reserve), "%s/.%s.reserve", primary->record_dir, out) != 0) {
            return -1;
        }
        ...
        if (fd >= 0) {
            ...
            if (avoid_public_store && avoid_public_store->public_dir[0]) {
                char p[HOLD_PATH_MAX];
                avoid_public_race = (hold_checked_snprintf(p, sizeof(p), "%s/%s.json", avoid_public_store->public_dir, out) == 0 && hold_path_exists(p));
            }
