...
int sigmund_init_user_store_from_home(const char *home, struct sigmund_store *store) {
    ...
    if (sigmund_checked_snprintf(store->base, sizeof(store->base), "%s/.local/state/sigmund", home) != 0 ||
        sigmund_checked_snprintf(store->record_dir, sizeof(store->record_dir), "%s", store->base) != 0 ||
        sigmund_checked_snprintf(store->log_dir, sizeof(store->log_dir), "%s", store->base) != 0 ||
        sigmund_checked_snprintf(store->console_dir, sizeof(store->console_dir), "%s/console", store->base) != 0 ||
        sigmund_checked_snprintf(store->profile_path, sizeof(store->profile_path), "%s/profiles.json", store->base) != 0 ||
        sigmund_checked_snprintf(store->alias_path, sizeof(store->alias_path), "%s/aliases.json", store->base) != 0) {
        return -1;
    }
    return 0;
}
...
static int init_user_store_for_current_user(struct sigmund_store *store) {
    ...
    return sigmund_init_user_store_from_home(home, store);
}
...
static int ensure_user_store_from_home_owned(const char *home, uid_t uid, gid_t gid, struct sigmund_store *store) {
    ...
    if (sigmund_init_user_store_from_home(home, store) != 0) {
        return -1;
    }
    if (sigmund_checked_snprintf(local_dir, sizeof(local_dir), "%s/.local", home) != 0 ||
        sigmund_checked_snprintf(state_dir, sizeof(state_dir), "%s/.local/state", home) != 0) {
        return -1;
    }
    if (sigmund_mkdir_p0700(store->base) != 0 ||
...
int sigmund_init_system_store(struct sigmund_store *store) {
    ...
    if (sigmund_checked_snprintf(store->base, sizeof(store->base), "%s", base) != 0 ||
        sigmund_checked_snprintf(store->record_dir, sizeof(store->record_dir), "%s/runs", base) != 0 ||
        sigmund_checked_snprintf(store->log_dir, sizeof(store->log_dir), "%s/logs", base) != 0 ||
        sigmund_checked_snprintf(store->public_dir, sizeof(store->public_dir), "%s/public", base) != 0 ||
        sigmund_checked_snprintf(store->console_dir, sizeof(store->console_dir), "%s/console", base) != 0 ||
        sigmund_checked_snprintf(store->profile_path, sizeof(store->profile_path), "%s/profiles.json", base) != 0 ||
        sigmund_checked_snprintf(store->alias_path, sizeof(store->alias_path), "%s/public/aliases.json", base) != 0) {
        return -1;
    }
    return 0;
}
...
int sigmund_ensure_system_store(struct sigmund_store *store) {
    if (sigmund_init_system_store(store) != 0) {
        return -1;
    }
...
static bool id_collides_in_store(const struct sigmund_store *store, const char *id, bool include_public) {
    ...
    if (sigmund_checked_snprintf(path, sizeof(path), "%s/%s.json", store->record_dir, id) == 0 && sigmund_path_exists(path)) {
        return true;
    }
    if (sigmund_checked_snprintf(path, sizeof(path), "%s/%s.log", store->log_dir, id) == 0 && sigmund_path_exists(path)) {
        return true;
    }
    if (sigmund_checked_snprintf(path, sizeof(path), "%s/.%s.reserve", store->record_dir, id) == 0 && sigmund_path_exists(path)) {
        return true;
    }
    if (store->console_dir[0] &&
        sigmund_checked_snprintf(path, sizeof(path), "%s/%s.sock", store->console_dir, id) == 0 && sigmund_path_exists(path)) {
        return true;
    }
    if (include_public && store->public_dir[0]) {
        if (sigmund_checked_snprintf(path, sizeof(path), "%s/%s.json", store->public_dir, id) == 0 && sigmund_path_exists(path)) {
            return true;
        }
    }
    return false;
...
int sigmund_gen_id_for_store(const struct sigmund_store *primary,
    ...
    for (int tries = 0; tries < 100; tries++) {
        if (sigmund_rand_bytes(b, sizeof(b)) != 0) {
            return -1;
        }
        ...
        if (avoid_public_store && avoid_public_store->public_dir[0]) {
            ...
            if (sigmund_checked_snprintf(p, sizeof(p), "%s/%s.json", avoid_public_store->public_dir, out) == 0 && sigmund_path_exists(p)) {
                continue;
            }
        }
        ...
        if (sigmund_checked_snprintf(reserve, sizeof(reserve), "%s/.%s.reserve", primary->record_dir, out) != 0) {
            return -1;
        }
        ...
        if (fd >= 0) {
            ...
            if (avoid_public_store && avoid_public_store->public_dir[0]) {
                char p[SIGMUND_PATH_MAX];
                avoid_public_race = (sigmund_checked_snprintf(p, sizeof(p), "%s/%s.json", avoid_public_store->public_dir, out) == 0 && sigmund_path_exists(p));
            }
