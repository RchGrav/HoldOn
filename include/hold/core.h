...
/*
 * HLOGIDX sidecar: per-record capture timestamps and stream metadata that
 * cannot be reconstructed from the plain log text. The viewer loads the map
 * (reloading as a followed log grows) and looks records up by their raw byte
 * offset — exactly the offset the filter engine reports per visible line.
 */
